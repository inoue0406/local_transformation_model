...
def eval_on_threshold(file_list,threshold,tdim_use,result_path,stat_size,test_tail):
    ...
    for i,infile in enumerate(file_list):
        ...
        i1 = int((rain_true.shape[3] - stat_size)/2)
        i2 = i1 + stat_size
        print("i1,i2=",i1,i2)
        rain_true = rain_true[:,:,:,i1:i2,i1:i2]
        rain_pred = rain_pred[:,:,:,i1:i2,i1:i2]
...
if __name__ == '__main__':
    ...
    if argc != 4:
        print('Usage: python main_hcncst_jma.py data_dir result_dir test_tail')
        quit()
