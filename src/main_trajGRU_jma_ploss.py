...
from scaler import *
from train_valid_epoch_ploss import *
...
if __name__ == '__main__':
    ...
    elif opt.model_name == 'trajgru_ploss':
        # trajGRU Euler-Lagrange Model
        from models_trajGRU.model_euler_lagrange_ploss import EF_el_ploss
        encoder_params,forecaster_params = model_structure_trajGRU(opt.image_size,opt.batch_size,"trajgru_el")
        encoder = Encoder(encoder_params[0], encoder_params[1]).to(device)
        forecaster = Forecaster(forecaster_params[0], forecaster_params[1],opt.tdim_use).to(device)
        model = EF_el_ploss(encoder, forecaster, opt.image_size, opt.pc_size, opt.batch_size, opt.model_mode, opt.interp_type).to(device)
