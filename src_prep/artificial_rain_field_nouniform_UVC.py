...
def simulate_UV_gp_field(scale,grid_size,nsamples,volume,seed):
    """
    Generate 2d random field
    scale : spatial scale of disturbance
    grid_size : grid size
...
def generate_rain_field(v,scale,volume,num):
    # initial values
    #scale = 0.1
    grid_size_gp = 50
    grid_size = 200
    ...
    u_grd=simulate_UV_gp_field(scale,grid_size_gp,nsamples,v[0],seed=num)
    v_grd=simulate_UV_gp_field(scale,grid_size_gp,nsamples,v[1],seed=num)
    ...
    for ns in range(nsamples):
        ...
        # velocity field
        vfield = np.zeros((2,grid_size,grid_size))
        xy_grd_f,vfield[0,:,:] = interp_finer_grid(xy_grd,u_grd[ns,:,:],grid_size)
        xy_grd_f,vfield[1,:,:] = interp_finer_grid(xy_grd,v_grd[ns,:,:],grid_size)
        # apply advection
        out_field = advection(xy_grd_f,field_f,vfield,dt,steps)
        # plotting
        case = "V%5.2f_%5.2f_S%5.3f_A%5.3f" % (v[0],v[1],scale,volume)
        print("case=",case)
        pic_path="../run/result_20200921_art_field_nouniform_UVC/%s/" % case
        plot_field(out_field,pic_path,case)
        
    # type
    out_field = out_field.astype(np.float32)
    vfield = vfield.astype(np.float32)
    cfield = np.ones((1,grid_size,grid_size)) # constant field
    # write
    outfile_root = "../data/artificial/nouniform_UVC/"
    ...
    h5file.create_dataset('C',data= cfield)
    h5file.close()  
...
if __name__ == '__main__':
    ...
    for num in range(istart,iend):
        ...
        generate_rain_field(velocity,scale,volume,num)
